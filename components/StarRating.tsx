...
function ModernStar({ type }: { type: "full" | "half" | "empty" }) {
  if (type === "full") {
    return (
      <svg className="w-5 h-5 text-blue-500 drop-shadow" fill="currentColor" viewBox="0 0 20 20">
  ...
  if (type === "half") {
    return (
      <svg className="w-5 h-5 text-blue-500 drop-shadow" viewBox="0 0 20 20">
        <defs>
  ...
  // empty
  return (
    <svg className="w-5 h-5 text-gray-300" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 20 20">
      <polygon points="10,2 12.59,7.36 18.51,7.97 14,12.14 15.18,18.02 10,15.1 4.82,18.02 6,12.14 1.49,7.97 7.41,7.36" />
    </svg>
  );
}
