...
export default function LeaderboardPage() {
  const [city, setCity] = useState<string>(mockCities[0]);
  ...
  return (
    <main className="max-w-2xl mx-auto py-10 px-4">
      ...
      {loading ? (
        ...
        <ul className="divide-y divide-gray-700 bg-black/60 rounded-xl shadow-lg overflow-hidden">
          {restaurants.map((resto, idx) => (
            <li
              key={resto.id}
              className={`flex items-center px-6 py-4 ${idx === 0 ? "bg-gradient-to-r from-yellow-400/20 to-transparent" : ""}`}
            >
